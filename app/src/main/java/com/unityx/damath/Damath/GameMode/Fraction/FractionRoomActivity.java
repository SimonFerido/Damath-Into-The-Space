...
public class FractionRoomActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private RecyclerViewAdapterFraction recyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fraction_room);
        clickSound = MediaPlayer.create(FractionRoomActivity.this, R.raw.click);
...
//**********************************
        ...
        refRoom = database.getReference("Room").child("fraction").child("available");
        refUnavailableRoom = database.getReference("Room").child("fraction").child("unavailable");
        ...
        refRoom.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                recyclerViewAdapter = new RecyclerViewAdapterFraction(FractionRoomActivity.this, roomManager.getRoomList());
        ...
        btnCreate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(FractionRoomActivity.this, BlueDamathFractionActivity.class);
                    ...
                    if (roomNum != 0) {
                        for (int i = 0; i < roomManager.getRoomList().size(); i++) {
                            if (roomNum == roomManager.getRoomList().get(i).getId()) {
                                ...
                                refRoom.child(String.valueOf(room.getId())).setValue(room);
                                Intent intent = new Intent(FractionRoomActivity.this, PinkDamathFractionActivity.class);
                                intent.putExtra("room", room);
                                FractionRoomActivity.this.startActivity(intent);
                                break;
                            }
                        }
                        if (exist == false) {
                            Toast.makeText(FractionRoomActivity.this, "Room Doesn't Exist", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
                else{//join random room
                    if(roomManager.getRoomList().size() == 0){
                        Toast.makeText(FractionRoomActivity.this, "No Available Rooms", Toast.LENGTH_SHORT).show();
                    }
                    else{
                        int min = 0;
