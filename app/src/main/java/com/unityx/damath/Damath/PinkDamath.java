...
public class PinkDamath extends Damath {

    private ArrayList <int[]> possibleMove;
    private ArrayList <int[]> killList;
    private static String type = "PinkDamath";

    public PinkDamath(int row, int column){
        super(row,column,type);
    }

    public PinkDamath (Damath pinkDamath){
        super(pinkDamath);
    }
    ...
    public void recursiveSearchNotKing(int r, int c, List<List<Damath>> damathList){
        if(c == 0 && r < 7) {
            if (!(damathList.get(r + 1).get(c + 1) instanceof PinkDamath)) { //if the redChecker is at column 0 and there is not one redChecker at lower right
            ...
            if(!(damathList.get(r + 1).get(c - 1) instanceof PinkDamath)){//if the lower left is not a redChecker
                if(damathList.get(r + 1).get(c - 1) instanceof BlueDamath){//if the lower left is a black checker
                    if ((r + 2 <= 7) && (c - 2 >= 0)) {
            ...
            if(!(damathList.get(r + 1).get(c + 1) instanceof PinkDamath)){//if the lower right is not a redChecker
                if(damathList.get(r + 1).get(c + 1) instanceof BlueDamath){//if the lower right is a black checker
                    if ((r + 2 <= 7) && (c + 2 <= 7)) {
