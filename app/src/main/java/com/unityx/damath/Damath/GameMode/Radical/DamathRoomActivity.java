...
public class DamathRoomActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private RecyclerViewAdapterRadical recyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_damath_room);
        clickSound = MediaPlayer.create(DamathRoomActivity.this, R.raw.click);
...
//**********************************
        ...
        refRoom = database.getReference("Room").child("radical").child("available");
        refUnavailableRoom = database.getReference("Room").child("radical").child("unavailable");
        ...
        refRoom.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                recyclerViewAdapter = new RecyclerViewAdapterRadical(DamathRoomActivity.this, roomManager.getRoomList());
        ...
        btnCreate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(DamathRoomActivity.this, BlueDamathRadicalActivity.class);
                    ...
                    if (roomNum != 0) {
                        for (int i = 0; i < roomManager.getRoomList().size(); i++) {
                            if (roomNum == roomManager.getRoomList().get(i).getId()) {
                                ...
                                refRoom.child(String.valueOf(room.getId())).setValue(room);
                                Intent intent = new Intent(DamathRoomActivity.this, PinkDamathRadicalActivity.class);
                                intent.putExtra("room", room);
                                DamathRoomActivity.this.startActivity(intent);
                                break;
                            }
                        }
                        if (exist == false) {
                            Toast.makeText(DamathRoomActivity.this, "Room Doesn't Exist", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
                else{//join random room
                    if(roomManager.getRoomList().size() == 0){
                        Toast.makeText(DamathRoomActivity.this, "No Available Rooms", Toast.LENGTH_SHORT).show();
                    }
                    else{
                        int min = 0;
